...
with open(args.config_file, mode="rb") as fp:
    c = tomli.load(fp)
    conf = {**c["preproc"]["slurm"], **c}
    del c

os.makedirs(f"{conf['general']['log_dir']}", exist_ok=True)

submit_script_preprocessing = f"""#!/bin/bash -l
#SBATCH -o {conf["general"]["log_dir"]}preproc.%A_%a.out
#SBATCH -e {conf["general"]["log_dir"]}preproc.%A_%a.err
#SBATCH -D ./
#SBATCH --constraint="apu"
#SBATCH -J nitronet_preproc_{nicename}
...
python preproc.py {args.config_file} $SLURM_ARRAY_TASK_ID
"""

id = uuid.uuid4().hex[:6]
submission_script_name = f'preproc_{id}.sh'
